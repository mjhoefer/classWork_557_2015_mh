...
GLSphereSpot::GLSphereSpot(float center_x, float center_y, float center_z, float radius, int rows, int segments, int sphereChoiceSelection )
{
	...
	sphereChoice = sphereChoiceSelection;
...
void GLSphereSpot::initShader(void)
{
...
#endif
	cout << "shpere choice is " << sphereChoice;
	switch (sphereChoice)
	{
	case 1:
		...
		_material._ambient_material = glm::vec3(0.0, 0.5, 0.0);
		...
		_light_source1._lightPos = glm::vec4(10.0, 0.0, 11.0, 1.0);
		_light_source1._ambient_intensity = 0.0;
		_light_source1._specular_intensity = 15.0;
		_light_source1._diffuse_intensity = 8.0;
		_light_source1._attenuation_coeff = 0.5;

		_light_source1._cone_angle = 30.0; // in degree
		_light_source1._cone_direction = glm::vec3(0.0, 0.25, -1.0); // this must be aligned with the object and light position.
	...
	case 2:
		_program = LoadAndCreateShaderProgram("../data/shaders/spotlightSmooth.vs", "../data/shaders/spotlight.fs");
		glUseProgram(_program);
		...
		_material._specular_material = glm::vec3(1.0, 0.5, 0.25);
		_material._shininess = 1.0;
		...
		// define the position of the light and send the light position to your shader program
		_light_source1._lightPos = glm::vec4(25.0, 0.0, 15.0, 1.0);
		_light_source1._ambient_intensity = 0.5;
		_light_source1._specular_intensity = 15.0;
		_light_source1._diffuse_intensity = 4.0;
		_light_source1._attenuation_coeff = 0.0;

		_light_source1._cone_angle = 10.0; // in degree
		float _cone_angle2 = 30.0; // degrees for smooth spotlight
		_light_source1._cone_direction = glm::vec3(0.0, 0.25, -1.0); // this must be aligned with the object and light position.
		...
		int _cone_angle2Idx = glGetUniformLocation(_program, "cone_angle2");
		...
		glUniform1f(_light_source1._cone_angleIdx, _light_source1._cone_angle);
		glUniform1f(_cone_angle2Idx, _cone_angle2);
		glUniform3fv(_light_source1._cone_directionIdx, 1, &_light_source1._cone_direction[0]);
		break;
	//default:
		//break;
