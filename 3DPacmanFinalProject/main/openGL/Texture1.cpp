...
//  modified by Yuanfen on 12/15/2015; enviroment mapping use unit 0, multitexure use unit 1 and 2
//
//
...
int GLMultiTexture::loadAndCreateTextures(string path_and_file_texture_1, string path_and_file_texture_2)
{
    ...
    // Create a texture and load it to your graphics hardware. This texture is automatically associated
    // with texture 1 and the textuer variable "texture" / the active texture.
    if(channels1 == 3)
...
bool GLMultiTexture::addVariablesToProgram(GLuint program, int variable_index)
{
    ...
    /*
     Then we set the tex uniform of the shaders to the index of the texture unit. We used texture unit one, so we set the tex uniform to the integer value 0.
     */
    glUniform1i(_textureIdx1, 1);
